...
const char DATETIME_FORMAT[] = "%Y-%m-%d %H:%M:%S %Z";
...
class Zone {
 public:
     explicit Zone(const std::string& tz_name) : _tz_name(tz_name) { }
 ...
 private:
     Breakdown breakdown() const {
         ...
         return {
             ...
             .hours = factor() * static_cast<int>(hours.count()),
             .minutes = factor() * static_cast<int>(minutes.count()),
             .seconds = factor() * static_cast<int>(seconds.count()),
             .millis = factor() * static_cast<int>(millis.count())
         };
     }
     ...
     static Date strptime(const std::string& date_str,
         ...
         if (struct_tm_is_empty(tm_date)) {
             THROW(core::ValueError,
                   "Could not parse Date from \"" + date_str + "\" with format string \"" + format + "\".");
         }
         return Date(tm_date);
     }
 ...
 public:
     ...
     static Datetime strptime(const std::string& dt_str,
         ...
         if (struct_tm_is_empty(tm_dt)) {
             THROW(core::ValueError,
                   "Could not parse Datetime from \"" + dt_str + "\" using format string \"" + format + "\".");
         }
