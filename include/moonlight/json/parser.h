             ...
             if (c == '\\') {
                 ...
                 if (iter != ESCAPED_CHARS.end()) {
                     ...
                     if (hexA == EOF || hexB == EOF) {
                         THROW(ParseError, "Unexpected end of file while parsing '\\x' escape sequence.",
                               context().loc());
                     }
                     std::string hex = {static_cast<char>(hexA), static_cast<char>(hexB)};
                     size_t pos;
...
class ObjectValueState : public State {
 public:
     explicit ObjectValueState(Value::Pointer obj) : _object(obj) { }
...
class ObjectState : public State {
 public:
     explicit ObjectState(Value::Pointer object) : _object(object) { }
...
class ArrayValueState : public State {
 public:
     explicit ArrayValueState(Value::Pointer array) : _array(array) { }
...
class ArrayState : public State {
 public:
     explicit ArrayState(Value::Pointer array) : _array(array) { }
...
class ValueState : public State {
 public:
     explicit ValueState(Value::Pointer* value_out) : value_out(value_out) { }
...
class Parser {
 public:
     explicit Parser(std::istream& in, const std::string& filename = "<input>")
     : ctx({
